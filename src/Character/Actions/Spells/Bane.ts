    ...
    for (let i = 0; i < this.targets.length; i += 1) {
      ...
      if (st < this.actor.character.spellCastingDc) {
        ...
        if (this.world.loggerCallback) {
          this.world.loggerCallback(`${this.actor.character.name} cast bane on ${this.targets[i].character.name}.`)
        }
      }
      else if (this.world.loggerCallback) {
        this.world.loggerCallback(`${this.actor.character.name} failed to cast bane on ${this.targets[i].character.name}.`)
